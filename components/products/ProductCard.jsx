...
export default function ProductCard({ product, viewMode = "grid" }) {
      ...
      <div className="bg-white rounded-xl sm:rounded-2xl border border-slate-200/80 p-3 sm:p-4 hover:shadow-lg transition-all duration-300 flex flex-col sm:flex-row gap-3 sm:gap-5 group">
        ...
        <div className="flex-1 flex flex-col justify-between">
          <div>
            ...
            <h3
              onClick={() => setQuickViewProduct(product)}
              className="text-sm sm:text-base font-bold text-slate-900 hover:text-emerald-600 cursor-pointer line-clamp-1 mb-1 sm:mb-1.5"
            >
              {product.title}
            </h3>

            <p className="text-xs text-slate-500 line-clamp-2 mb-2 sm:mb-3">
              {product.description}
            </p>
            ...
            <div className="flex flex-wrap items-center gap-1.5 sm:gap-2 mb-2">
              ...
              {product.tags?.slice(0, 2).map((tag, idx) => (
                <span
                  key={idx}
                  className="text-[10px] sm:text-[11px] font-medium text-slate-600 bg-slate-100 px-2 py-0.5 rounded-md"
                >
                  {tag}
                </span>
              ))}
          ...
          {/* Pricing & Actions */}
          <div className="flex items-center justify-between pt-2.5 sm:pt-3 border-t border-slate-100 mt-2">
            ...
            <div className="flex items-center gap-1.5 sm:gap-2">
              <button
                ...
                className="p-2 sm:p-2.5 rounded-xl border border-slate-200 text-slate-600 hover:text-rose-600 hover:border-rose-200 transition-colors"
                title="Wishlist"
              >
              ...
              <button
                onClick={() => setQuickViewProduct(product)}
                className="p-2 sm:p-2.5 rounded-xl border border-slate-200 text-slate-600 hover:text-emerald-600 hover:border-emerald-200 transition-colors"
                title="Quick View"
              >
              ...
              <button
                ...
                className="bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold px-3 sm:px-4 py-2 sm:py-2.5 rounded-xl flex items-center gap-1.5 transition-colors shadow-sm"
              >
  ...
  return (
    <div className="bg-white rounded-2xl sm:rounded-3xl border border-slate-200/80 p-2.5 sm:p-4 hover:shadow-xl hover:border-slate-300 transition-all duration-300 flex flex-col justify-between group relative">
      ...
      {/* Product Image */}
      <div
        className="relative aspect-square rounded-xl sm:rounded-2xl overflow-hidden bg-slate-50 mb-2 sm:mb-3.5 cursor-pointer"
        onClick={() => setQuickViewProduct(product)}
      >
        <img
          src={product.image}
        ...
        <div className="hidden sm:block absolute inset-x-3 bottom-3 opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-y-2 group-hover:translate-y-0">
          <button
            onClick={(e) => {
              e.stopPropagation();
        ...
        <div>
          ...
          {vendor && (
            <div className="flex items-center justify-between mb-0.5 sm:mb-1">
              <button
                onClick={() => setActiveVendorId(vendor.id)}
                className="text-[10px] sm:text-[11px] font-semibold text-emerald-600 hover:text-emerald-800 hover:underline line-clamp-1 flex items-center gap-1 text-left"
              >
                <span>{vendor.name}</span>
                {vendor.verified && (
                  <CheckCircle2 className="w-2.5 h-2.5 sm:w-3 sm:h-3 fill-emerald-600 text-white shrink-0" />
                )}
              </button>
            </div>
          )}
          ...
          <h3
            onClick={() => setQuickViewProduct(product)}
            className="text-xs sm:text-sm font-bold text-slate-900 group-hover:text-emerald-600 cursor-pointer line-clamp-2 mb-1 sm:mb-2 leading-tight sm:leading-snug"
          >
            {product.title}
          </h3>

          {/* Rating */}
          <div className="flex items-center gap-1 sm:gap-1.5 mb-2 sm:mb-3 text-[11px] sm:text-xs">
            ...
            {product.stock <= 15 && (
              <span className="hidden xs:inline-block ml-auto text-[10px] font-bold text-amber-700 bg-amber-50 px-1.5 py-0.5 rounded">
